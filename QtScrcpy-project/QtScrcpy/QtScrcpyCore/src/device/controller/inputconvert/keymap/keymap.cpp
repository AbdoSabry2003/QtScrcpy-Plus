...
void KeyMap::loadKeyMap(const QString &json)
{
    ...
    if (checkItemObject(rootObj, "mouseMoveMap")) {
        ...
        if (checkItemDouble(mouseMoveMap, "maxOffsetX")) {
            keyMapNode.data.mouseMove.maxOffsetX = getItemDouble(mouseMoveMap, "maxOffsetX");
        }
        if (checkItemDouble(mouseMoveMap, "maxOffsetY")) {
            keyMapNode.data.mouseMove.maxOffsetY = getItemDouble(mouseMoveMap, "maxOffsetY");
        }

        if (checkItemDouble(mouseMoveMap, "recenterDelayMs")) {
            keyMapNode.data.mouseMove.recenterDelayMs = static_cast<int>(getItemDouble(mouseMoveMap, "recenterDelayMs"));
        }

        if (mouseMoveMap.contains("dualTouchMode") && mouseMoveMap.value("dualTouchMode").isString()) {
            QString mode = mouseMoveMap.value("dualTouchMode").toString();
            if (mode == "clean") keyMapNode.data.mouseMove.dualTouchMode = 1;
            else if (mode == "delayed") keyMapNode.data.mouseMove.dualTouchMode = 2;
            else if (mode == "overlap") keyMapNode.data.mouseMove.dualTouchMode = 3;
            else keyMapNode.data.mouseMove.dualTouchMode = 0;
        }

        // extraEyes
        m_extraEyes.clear();
        if (mouseMoveMap.contains("extraEyes") && mouseMoveMap.value("extraEyes").isArray()) {
            QJsonArray eyesArray = mouseMoveMap.value("extraEyes").toArray();
            for (int i = 0; i < eyesArray.size(); ++i) {
                if (!eyesArray[i].isObject()) continue;
                QJsonObject eyeObj = eyesArray[i].toObject();

                QPair<ActionType, int> key = getItemKey(eyeObj, "key");
                if (key.first == AT_INVALID) continue;

                ExtraEye eye;
                eye.keyType = key.first;
                eye.key = key.second;
                eye.pos = getItemPos(eyeObj, "pos");

                if (checkItemDouble(eyeObj, "maxOffsetX")) {
                    eye.maxOffsetX = getItemDouble(eyeObj, "maxOffsetX");
                }
                if (checkItemDouble(eyeObj, "maxOffsetY")) {
                    eye.maxOffsetY = getItemDouble(eyeObj, "maxOffsetY");
                }

                m_extraEyes.push_back(eye);
            }
        }
