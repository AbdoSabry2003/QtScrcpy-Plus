...
export class ScaleManager {
    ...
    _updateMouseMovePosition(node) {
        ...
        if (mouseShape) {
            node.shape.position(denormalized);
            mouseShape.position({ x: 0, y: 0 });
        }

        // Update boundary rectangle
        if (node.updateBoundaryRect) {
            node.updateBoundaryRect();
        }
    }

    _updateEyePosition(node) {
        const pos = node.mappingData?.pos;
        if (!pos) return;

        const denormalized = this.denormalizePosition(pos);
        const eyeShape = node.shape.findOne('.eyeShape');

        if (eyeShape) {
            node.shape.position(denormalized);
            eyeShape.position({ x: 0, y: 0 });
        }
